...
This file provides guidance to Claude Code and other agentic coding tools when working with code in this repository.
...
- **ApplicationDockerService** — Child model of `Application` for compose buildpack apps. One row tracks each compose service and its live container status.
- **Service** — A pre-configured service stack from templates (`templates/service-templates-latest.json`). Uses `ServiceApplication` / `ServiceDatabase` child models for per-service management, including individual env vars, domains, backups, settings, autoscaling, and monitoring.
...
- `coolify-design` — Coolify UI/UX and design system (Livewire, Blade, Alpine, Tailwind). Activate for any frontend visual work; follows `DESIGN.md`.
